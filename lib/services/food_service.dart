...
class FoodService {
  ...
  Future<int> insert(Food food) async {
    ...
    return db.insert(
      Food.tblFood, food.toMap(),
      conflictAlgorithm:
          ConflictAlgorithm.ignore, //ignores conflicts due to duplicate entries
    );
  }

  Future<int> update(int id, Food food) async {
    // returns the number of rows updated
    Database db = await _dbHelper.database;
    int result = await db.update(Food.tblFood, food.toMap(),
        where: '${Food.colId} = ?', whereArgs: [id]);
    return result;
  }
  ...
  Future<int> delete(int id) async {
    ...
    Database db = await _dbHelper.database;
    int result = await db.delete(Food.tblFood,
        where: '${Food.colId} = ?',
